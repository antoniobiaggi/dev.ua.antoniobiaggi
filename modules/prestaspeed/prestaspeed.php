...
class PrestaSpeed extends Module
{
    ...
    public function install()
    {
        if (!Configuration::updateValue(
            ...
            'watermark'
        ) || !$this->registerHook('footer') || !$this->updatePosition(Hook::getIdByName('actionWatermark'), 0, 1)
        ) {
            return false;
        }
        if (!Configuration::updateValue(
	...
	*/
    ...
    public function hookFooter()
    {
        ...
        $finish = $time;
        $total_time = round(
            ($finish - Configuration::get('PRESTASPEED_LOADS')),
            4
        );
        Configuration::updateValue(
            'PRESTASPEED_LOAD',
    ...
    public function minIfin($wtm)
    {
        ...
        //$files3 = $this->rglob('{*.'.$wtm.'}', '../modules/', GLOB_BRACE);
        $files4 = $this->rglob('{*.'.$wtm.'}', '../js/', GLOB_BRACE);
        $files2 = array_merge($files, $files4);
        $mini = new Minifier();
    ...
    public function renderFormO()
    {
        ...
        $fields_form = array(
            'form' => array(
                ...
                'description' => $this->l('If you get a timeout when minify CSS/JS, just Press F5 to continue, or compress first CSS and later JS.If you have errors when minify CSS / JS files, use this URL to restore the original files: ')._PS_BASE_URL_._MODULE_DIR_.'prestaspeed/prestaspeed-minify.php?token='.Tools::substr(Tools::encrypt('prestaspeed/minify'), 0, 10).'&id_shop='.$this->context->shop->id,
    ...
    public function renderForm2()
    {
        ...
        $fields_form = array(
            'form' => array(
                ...
                'description' => $this->l('Total KB optimized in images: ').round($datatot, 2).'<br/>'.$this->l('Total images to process: ').Configuration::get('PRESTASPEED_TOTFIL').'<br/>'.$this->l('Total images processed: ').Configuration::get('PRESTASPEED_TOTSMUSH').'<br/>'.(Configuration::get('PRESTASPEED_TOTFIL') != Configuration::get('PRESTASPEED_TOTSMUSH') ? $this->l('WARNING: The module does not process all the images. Please optimize the images again to complete the process') : $this->l('All images were processed')).'<br/><br/>'.$this->l('PrestaSpeed generates a backup of every image optimized with the -old extension.').'<br/>'.$this->l('Use this cron url to delete backup images in the img folder, or click on the link to execute now: ')._PS_BASE_URL_._MODULE_DIR_.'prestaspeed/prestaspeed-remove.php?token='.Tools::substr(Tools::encrypt('prestaspeed/remove'), 0, 10).'&id_shop='.$this->context->shop->id.'<br/><br/>'.$this->l('If the optimization process fail and you get empty images, you can restore the images with this url:')._PS_BASE_URL_._MODULE_DIR_.'prestaspeed/prestaspeed-restore.php?token='.Tools::substr(Tools::encrypt('prestaspeed/restore'), 0, 10).'&id_shop='.$this->context->shop->id,
                'input' => array(
                    array(
                        'type' => 'switch',
    ...
    <label>'.$this->l('Clean Pages not found:').'</label>
        ...
        <div class="margin-form">
            <input class="datepicker" type="text" name="befo" value="'.Tools::getValue('befo', Configuration::get('PRESTASPEED_BEFO')).'" style="text-align: center" id="befo" />
			                            <p class="clear">'.$this->l('Set a date to delete all connections and pagenotfound data before that date. If you get a white screen, start from old dates, and increase to new ones in each optimization').'</p>
...
<br/><p>'.$this->l('If the optimization process fail and you get empty images, you can restore the images with this url:')._PS_BASE_URL_._MODULE_DIR_.'prestaspeed/prestaspeed-restore.php?token='.Tools::substr(Tools::encrypt('prestaspeed/restore'), 0, 10).'&id_shop='.$this->context->shop->id.'</p><br/>
     ...
     <br/>
           <form action="'.$_SERVER['REQUEST_URI'].'" method="post" enctype="multipart/form-data">
     <fieldset>
      <legend><img src="'.$this->_path.'views/img/pictures.gif" alt="" title="" />'.$this->l('Optimize images in a folder').'</legend>
...
display:none
            ...
            ).$this->renderForm2().$this->renderForm3().$this->renderForm4().$this->_displayInfo();
        }
    ...
    public function log()
    {
        ...
        foreach ($files as $file) {
            // echo 'Log File: <a href="'.str_replace('../','http://'.$_SERVER['HTTP_HOST'].__PS_BASE_URI__.'modules/',$file).'" target="_blank"> '.$file.'</a> - File Size:'.filesize($file).' bytes </br>';
