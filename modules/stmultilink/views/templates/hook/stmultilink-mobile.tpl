...
<ul id="multilink_mobile_{$link_group.id_st_multi_link_group}" class="mo_mu_level_0 mobile_menu_ul">
    <li class="mo_ml_level_0 mo_ml_column">
        <a href="{if $link_group.url}{$link_group.url|escape:'html'}{else}javascript:;{/if}" rel="nofollow" class="mo_ma_level_0 {if !$link_group.url}ma_span{/if}"{if isset($link_group.new_window) && $link_group.new_window} target="_blank" {/if}>
        ...
        {foreach $link_group['links'] as $link}
            <li class="mo_ml_level_1 mo_sub_li">
                <a href="{$link.url|escape:'html'}" title="{$link.title|escape:html:'UTF-8'}" {if isset($link.nofollow) && $link.nofollow} rel="nofollow" {/if} {if $link.new_window} target="_blank" {/if} class="mo_ma_level_1 mo_sub_a">
