...
class RelatedProductPro extends Module
{
        ...
        if ($id_product) {
            ...
            $this->smarty->assign(
                array(
                    ...
                    'version_16' => (bool)version_compare(_PS_VERSION_, '1.6.0.0 ', '>='),
                    'id_shop' => $id_shop,
    ...
    public function hookActionProductUpdate($params)
    {
        if (Tools::isSubmit('related_product_pro')) {
            ...
            $related_product = Tools::getValue('related_product_input');
            if (!$id_shop) {
            ...
            if ($id_product) {
                $relateds = new RelatedProductProModel($id_product, $id_shop);
                $relateds->relatedUpdate($related_product);
            }
        }
    }
    ...
    public function hookDisplayFooterProduct()
    {
        ...
        foreach ($product_data as $product) {
            ...
            $p = $relateds->getProductData($product['id_related']);
            if (!empty($p)) {
                $products[] = $p;
            }
            $i++;
