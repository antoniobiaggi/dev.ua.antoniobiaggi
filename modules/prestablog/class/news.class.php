...
class NewsClass extends ObjectModel
{
	...
	public function registerTablesBdd()
	{
		...
		if (!Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('
		CREATE TABLE `'.bqSQL(_DB_PREFIX_.$this->table).'_lang` (
		...
		if (!Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('
		CREATE TABLE `'.bqSQL(_DB_PREFIX_.$this->table).'_product` (
		...
		if (!Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('
		CREATE TABLE `'.bqSQL(_DB_PREFIX_).'prestablog_news_newslink` (
