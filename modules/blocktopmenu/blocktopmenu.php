...
class Blocktopmenu extends Module
{
    ...
    {
		global $smarty;
		$site_url = $smarty->tpl_vars['base_dir']->value;
        $html = '';
        $hasClass = '<li class="has-thumb">';
		...
		foreach ($categories as $key => $category) {
			...
			$photo = $site_url.'img/c/'.$category['id_category'].'_thumb.jpg';

			if (file_exists('./img/c/'.$category['id_category'].'_thumb.jpg')){
				$hasClass .= '<div><img src="'.$photo.'" alt="'.$category['name'].'" class="imgm">';
				$array_name[] = $category['name'];
				$hasClass .= '<a href="'.$link.'">'.$category['name'].'</a>';
                $hasClass .='</div>';
			}
		}
		
		$hasClass .= '</li>';
		if ($hasClass == '<li class="has-thumb"></li>'){
			$hasClass = '';
		}
        foreach ($categories as $key => $category) {
            if ($category['level_depth'] > 1) {
			...
			if (!in_array($category['name'],$array_name)){
				$html .= '<li'.(($this->page_name == 'category'
					&& (int)Tools::getValue('id_category') == (int)$category['id_category']) ? ' class="sfHoverForce"' : '').'>';
				$html .= '<a href="'.$link.'">'.$category['name'].'</a>';
				
				if (isset($category['children']) && !empty($category['children'])) {
					$html .= '<div class="subholder"><ul class="sub clearfix">';
						...
						//if (count(preg_grep('/^'.$category['id_category'].'-([0-9])?_thumb.jpg/i', $files)) > 0) {
							...
							foreach ($files as $file) {
								...
								if (preg_match('/^'.$category['id_category'].'_thumb.jpg/i', $file) === 1) {
								   ...
								   // .'" alt="'.Tools::SafeOutput($category['name']).'" class="imgm" />';
								}
							}
							 $html .= '<div>';
					...
					$html .= '<div class="more-catalogue"><a href="/30-vsya-kollekciya">ВСЯ КОЛЛЕКЦИЯ<i class="icon-n fa fa-arrow-circle-right"></i></a></div>';
					$html .= '</ul></div>';
				}
			}
            $html .= '</li>';
