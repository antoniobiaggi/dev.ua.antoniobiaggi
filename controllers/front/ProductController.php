...
class ProductControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        if (!$this->errors) {
            ...
            $result = Db::getInstance()->executeS('SELECT id_product_pack FROM `'._DB_PREFIX_.'pack` WHERE id_product_item = '.(int)$this->product->id);
                
            $pack_items_custom = array();
            $merge = array();
            foreach ($result as $id){
                $pack_items_custom[] = Pack::isPack($id['id_product_pack']) ? Pack::getItemTable($id['id_product_pack'], $this->context->language->id, true) : array();
                
            } 
			
			foreach ($pack_items_custom as &$pac){
				foreach ($pac as $k => &$v){
                $v['attributes'] = $this->product->getAttributesGroupsCustom($this->context->language->id, $v['id_product']);    
					if ($v['id_product'] == Tools::getValue('id_product')){
						list($pac[0],$pac[$k]) = array($pac[$k],$pac[0]);
					}
				}
			}
            ...
            $this->context->smarty->assign('packItemsMainProd', $pack_items_custom);
    ...
    protected function assignAttributesGroups()
    {
        ...
        if (is_array($attributes_groups) && $attributes_groups) {
			...
			//echo '</pre>';
			foreach ($groups as &$group) {
				if ($group['group_name'] == 'Shoes Size'){
					if ($group['default'] != -1){
						$noNullArray = true;
						break;
					}
			...
			if (!$noNullArray){
				foreach ($groups as &$group) {
					if ($group['group_name'] == 'Shoes Size'){
						$group['default'] = array_keys($group['attributes_quantity'])[0];
					}else{
						$group['default'] = 11;
					}             
				}
