...
class AuthControllerCore extends FrontController
{
    ...
    protected function processSubmitAccount()
    {
		...
		if (Tools::getValue('choosedelivery') == '0') {
            $this->errors['choosedelivery'] = Tools::displayError('Выберите доставку.');
        }elseif(Tools::getValue('choosedelivery') == 'mex'){
			if (!Tools::getValue('city')) {
				$this->errors['city'] = Tools::displayError('Город обязательное поле пропущено.');
			}
			if (!Tools::getValue('address1')) {
				$this->errors['address1'] = Tools::displayError('Адрес обязательное поле пропущено.');
			}
		}	
        ...
        if ($error_phone) {
            $this->errors['phone_mobile'] = Tools::displayError('You must register at least one phone number.');
        }
