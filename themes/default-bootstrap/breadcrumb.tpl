...
<div class="catalog-nav">
	<a class="catalog-darken" href="{if isset($force_ssl) && $force_ssl}{$base_dir_ssl}{else}{$base_dir}{/if}" title="{l s='Return to Home'}">{l s='Home'}</a>
	
	{if isset($path) AND $path}
<span class="catalog-line">/</span>
		{if $path|strpos:'span' !== false}
			<span class="catalog-line">{$path|@replace:'<a ': '<span itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a class="catalog-lighten" itemprop="url" '|@replace:'data-gg="">': '<span itemprop="title">'|@replace:'</a>': '</span></a></span>'}</span>
		{else}
{$path}
		{/if}
	{/if}
