...
-->
    <h1 class="send-msg">{l s='Контакты'}</h1>
				<!-- arrow -->
				<div class="arrowDecor contDecorM"></div>
				<!-- /arrow -->
				<!-- blocks -->
				<div class="blocksWrap contFloat">
					<h2>{l s='Обслуживание Покупателей - Связаться с нами'}</h2>
					<div class="contInfo">
					    <p class="payText contMar"><strong>{l s='Компания ООО «АНТОНИО БИАДЖИ - УКРАИНА»'}</strong></p>
					    <p class="payText contMar">{l s='г. Днепропетровск, Украина'}</p>
					    <p class="payText contMar25"><span><strong>{l s='+38 (056) 790-86-43'}</strong></span></p>
					    <p class="payText contMar">{l s='Горячая линия '}<span><strong>{l s='0 (800) 301-041'}</strong></span></p>
					    <p class="payText contMar">{l s='Работает с понедельника по пятницу с'} <strong>{l s='8:00'}</strong>{l s=' до '}<strong>{l s='20:00'}</strong></p>
					    <p class="payText contMar25">{l s='в субботу и воскресенье – с '}<strong>{l s='10:00'}</strong>{l s=' до '}<strong>{l s='18:00'}</strong>.</p>
					    <div class="email">{l s='Email'} <a href="mailto:shop@antoniobiaggi.com" class="contLink">{l s='shop@antoniobiaggi.com'}</a></div>
					</div>
					<form action="#" id="contForm">
                        <p class="payText upper"><strong>{l s='Ответим на Ваш запрос'}</strong></p>
                        <p class="payText upper mb17"><strong>{l s='при первой же возможности'}</strong></p>
                        <input name="Name" type="text" placeholder="Имя и Фамилия (обязательно)" required>
                        <input name="Email" type="email" required placeholder="Email (обязательно)">
                        <input name="Phone" type="text" placeholder="Телефон (не обязательно)">
                        <input name="Themes" type="text" placeholder="Тема">
                        <textarea name="Message" id="contTxtarea" cols="30" rows="10" placeholder="Сообщение" ></textarea>
                        <button class="contFormBtn" type="submit">{l s='ОТПРАВИТЬ'}</button>
					</form>
