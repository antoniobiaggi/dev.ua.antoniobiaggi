...
{if isset($p) AND $p}
	...
	<!-- Pagination -->
	    ...
	    {if $nb_products > $products_per_page && $start!=$stop}
			<form class="showall catalog-show" action="{if !is_array($requestNb)}{$requestNb}{else}{$requestNb.requestUrl}{/if}" method="get">
		...
		{if $start!=$stop}
         ...
         <li>   <a href="{$link->goPage($requestPage, $p_previous)}" rel="prev" class="catalog-arrowL"></a></li>
				{else}
				<li>	 <a href="{$link->goPage($requestPage, $p_previous)}" rel="prev" class="catalog-arrowL"></a></li>
				{/if}
				{if $start==3}
				...
				{if $pages_nb > 1 AND $p != $pages_nb}
					...
					<li><a  href="{$link->goPage($requestPage, $p_next)}" rel="next" class="catalog-arrowR"></a></li>
				{else}
					<li><a  href="{$link->goPage($requestPage, $p_next)}" rel="next" class="catalog-arrowR"></a></li>
				{/if}
			</ul>
