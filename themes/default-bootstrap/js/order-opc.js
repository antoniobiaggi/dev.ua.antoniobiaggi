	...
	if ((typeof isLogged == 'undefined' || !isLogged) || (typeof isGuest !== 'undefined' && isGuest))
	{
		...
		$(document).on('click', '#submitAccount, #submitGuestAccount', function(e){
			...
			if ($('#deliv').text() == 'МЕТОД ДОСТАВКИ ▾'){
				params += 'choosedelivery=0&';
			}
			
			if ($('.my_np').css('display') == 'none' && $('#deliv').text() != 'МЕТОД ДОСТАВКИ ▾'){
				params += 'choosedelivery=mex&';
			}
			if ($('#deliv').text().trim().indexOf('Новая Почта') != -1){
				params += 'choosedelivery=np&';
			}
			...
			$.ajax({
				...
				success: function(jsonData)
				{
					...
					if (jsonData.hasError)
					{
						...
						var i = 0;
						for(var error in jsonData.errors){
							if (error == 'choosedelivery'){
								$('#deliv').css({
                                    'border-color':'red'
                                });
							}
							if (error == 'phone_mobile'){
								$('#phone_mobile').css({
                                    'border-color':'red'
                                });
							}
