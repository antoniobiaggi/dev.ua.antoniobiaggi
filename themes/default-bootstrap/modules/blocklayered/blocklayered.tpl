...
<div class="catalogFilters">
							<div class="addFilters">
								<div class="icon"></div>
								<div class="text"> {l s="filters"}</div>
							</div>
							<div class="deleteFilters">
								<div class="icon"></div>
								<div class="text"> {l s="hide filters"}</div>
							</div>
						</div>
			...
			<div>
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						...
						{/if}
                        <div class="catalogue-selectName">
						...
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
							{if !isset($filter.slider)}
								{if $filter.filter_type == 0}
									{foreach from=$filter.values key=id_value item=value name=fe}
										...
										<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} col-lg-12  {if isset($value.checked) && $value.checked && isset($filter.is_color_group) && $filter.is_color_group}on{/if}">
								
											{if isset($filter.is_color_group) && $filter.is_color_group}
												<input value="{$value.name}" class="color-option {if !$value.nbr}disable{/if}" type="button" name="layered_{$filter.type_lite}_{$id_value}" data-rel="{$id_value}_{$filter.id_key}" id="layered_id_attribute_group_{$id_value}" {if !$value.nbr}disabled="disabled"{/if} style="background: {if isset($value.color)}{if file_exists($smarty.const._PS_ROOT_DIR_|cat:"/img/co/$id_value.jpg")}url(img/co/{$id_value}.jpg){else}{$value.color}{/if}{else}#CCC{/if};" />
											...
											<label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled "{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color " data-rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
												...
												{else}
												<a class="disc" href="{$value.link}"{if $value.rel|trim != ''} data-rel="{$value.rel}"{/if}>{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}</a>
												{/if}
											</label>
										</li>
										{/if}
                            ...
                            $( "#myslider").slider({
                                    ...
                                    step : 1,//Шаг, с которым будет двигаться ползунок
                                       range: true,
                                        values: [ $("#layered_price_range_min").val(),                             $("#layered_price_range_max").val()
                                                ],
                                    create: function( event, ui ) {
                                       val = $( "#myslider" ).slider("values");
                                    },
                                slide: function( event, ui ) {
                                  $("#layered_price_range_min").val($('#myslider').slider('values', 0));
                                   $("#layered_price_range_max").val($('#myslider').slider('values', 1));
                                     reloadContent();
...
//                var min = $('#layered_price_range_min').val();
//                var max = $('#layered_price_range_max').val();
//                    $('#myslider').slider({
//                        range: true,
//                        values: [$('#layered_price_range_min').val(), $('#layered_price_range_max').val()],
...
//                    });
									...
									<li class="nomargin row">
	                                    <div class="col-xs-6 col-sm-12 col-lg-6 first-item w42 ml15">
	                                    ...
	                                    <div class="minus"></div>
	                                    <div class="col-xs-6 col-sm-12 col-lg-6 w42">
