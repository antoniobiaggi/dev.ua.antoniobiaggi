...
<!--		<label for="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}"></label>-->
		<select id="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="selectProductSort form-control catalog-option">
			<option value="{if $page_name != 'best-sales'}{$orderbydefault|escape:'html':'UTF-8'}:{$orderwaydefault|escape:'html':'UTF-8'}{/if}"{if !in_array($orderby, array('price', 'name', 'quantity', 'reference')) && $orderby eq $orderbydefault} selected="selected"{/if}>{l s="Sort by price"}</option>
			{*}
			{if !$PS_CATALOG_MODE}
			...
			{/if}
			{*}
			<option value="reference:asc"{if $orderby eq 'reference' AND $orderway eq 'asc'} selected="selected"{/if}>{l s='Lowest first'}</option>
			<option value="reference:desc"{if $orderby eq 'reference' AND $orderway eq 'desc'} selected="selected"{/if}>{l s='Highest first'}</option>
		</select>
<!--	</div>-->
</form>
<!-- /Sort products -->
