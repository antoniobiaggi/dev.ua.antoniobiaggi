<div id="opc_new_account" class="opc-main-block basket_right">
	<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="new_account_form" class="std" autocomplete="on" autofill="on">
				<div id="opc_account_form" class="basket_right form_left">
				<div class="basket_form_name">{l s='Delivery & checkout'}</div>
				<div class="basket_form_desc">{l s='Already registered? '}<a href="/authentication?back=my-account">{l s='Sign in'}</a> {l s='for faster checkout.'}</div>
				    ...
				    <input required type="text" class="text form-control validate" id="customer_firstname" name="customer_firstname" onblur="$('#firstname').val($(this).val());" data-validate="isName" value="{if isset($guestInformations) && isset($guestInformations.customer_firstname) && $guestInformations.customer_firstname}{$guestInformations.customer_firstname}{/if}" />
						<label for="firstname">{l s='First name'}</label>	
					</div>
					<div class="required form-group">	
						<input required type="text" class="form-control validate" id="customer_lastname" name="customer_lastname" onblur="$('#lastname').val($(this).val());" data-validate="isName" value="{if isset($guestInformations) && isset($guestInformations.customer_lastname) && $guestInformations.customer_lastname}{$guestInformations.customer_lastname}{/if}" />
						<label for="lastname">{l s='Last name'}</label>
					</div>
				    <div class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group">
						<input  required  type="text" class="text form-control validate" name="phone_mobile" id="phone_mobile" data-validate="isPhoneNumber" value="{if isset($guestInformations) && isset($guestInformations.phone_mobile) && $guestInformations.phone_mobile}{$guestInformations.phone_mobile}{/if}" />
						<label for="phone_mobile">{l s="phone"}</label>
					</div>
					<div class="required text form-group">
						<input type="text" required class="text form-control validate" id="email" name="email" data-validate="isEmail" value="{if isset($guestInformations) && isset($guestInformations.email) && $guestInformations.email}{$guestInformations.email}{/if}" />
						<label for="email">{l s='Email'}</label>
					</div>
					<div class="password is_customer_param form-group" style="height:0px;visibility:hidden;">
						<label for="passwd">{l s='Password'} <sup>*</sup></label>
						<input type="password" class="text form-control validate" name="passwd" value="{$string}" id="passwd" data-validate="isPasswd" />
...
<!--							<label for="address1">{l s='Address'}</label>-->
				    ...
				    <div class="form-group is_customer_param">
						<textarea placeholder='{l s="Additional information"}'  class="form-control" name="other" id="info_text"></textarea>
					</div>
				</div>
...
<input type="checkbox" id="basket_ckeck">
                        <label for="basket_ckeck">{l s='I agree to'}<a href="/content/21-termsConditions">{l s='User Agreement'}</a>
                        </label>
							</div>
...
-->
					<div id="opc_invoice_address" class="is_customer_param">
						...
						<div class="required form-group">
							...
							<input type="text" class="form-control validate" id="lastname_invoice" name="lastname_invoice" data-validate="isLastName" value="{if isset($guestInformations) && isset($guestInformations.lastname_invoice) && $guestInformations.lastname_invoice}{$guestInformations.lastname_invoice}{/if}" />
						</div>
						{elseif $field_name eq "address1"}
						<div class="required form-group">
						...
						<div class="required form-group">
							...
							<select name="id_country_invoice" id="id_country_invoice" class="form-control">
								<option value="">-</option>
<!--								{foreach from=$countries item=v}-->
				<option value="216" selected="selected">{$v.name|escape:'html':'UTF-8'}</option>
<!--								{/foreach}-->
							</select>
						</div>
						{elseif $field_name eq "state" || $field_name eq 'State:name'}
...
-->
						<button type="submit" name="submitAccount" id="submitAccount" class="btn btn-default button button-medium">{l s='Способы оплаты'}</button>
						<a class="contshopping" href="/3-dlya-zhensshin">{l s='CONTINUE SHOPPING'}</a>
