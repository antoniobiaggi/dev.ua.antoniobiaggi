...
class ProductCore extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'main_prod' =>                  array('type' => self::TYPE_BOOL,'validate' => 'isBool'),
    ...
    */
        ...
        $id_customer = 0, $use_customer_price = true, $id_cart = 0, $real_quantity = 0)
    {
        ...
        if (!isset(self::$_pricesLevel2[$cache_id_2])) {
            $sql = new DbQuery();
            $sql->select('product_shop.`price`, p.`cache_is_pack`, product_shop.`ecotax`');
            $sql->from('product', 'p');
		...
		$result = Db::getInstance()->executeS('SELECT cache_is_pack FROM `'._DB_PREFIX_.'product` WHERE id_product = '.(int)$id_product);
		if ($result[0]['cache_is_pack'] == 1){
			$pack = Db::getInstance()->executeS('SELECT id_product_item FROM `'._DB_PREFIX_.'pack` WHERE id_product_pack = '.(int)$id_product);
			foreach ($pack as $p){
    ...
    */
    ...
    public function getAttributesGroupsCustom($id_lang,$id_prod)
    {
        if (!Combination::isFeatureActive()) {
            return array();
        }
        $sql = 'SELECT ag.`id_attribute_group`, ag.`is_color_group`, agl.`name` AS group_name, agl.`public_name` AS public_group_name,
                ...
                LEFT JOIN `'._DB_PREFIX_.'attribute_group_lang` agl ON (ag.`id_attribute_group` = agl.`id_attribute_group`)
                '.Shop::addSqlAssociation('attribute', 'a').'
                WHERE pa.`id_product` = '.(int)$id_prod.'
                    AND ag.`id_attribute_group` = 2
