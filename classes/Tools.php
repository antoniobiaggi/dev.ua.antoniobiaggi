...
class ToolsCore
{
    ...
    */
    ...
    public static function getProductObjectById($id_product){
        $product = new Product($id_product, true, Context::getContext()->language->id, Context::getContext()->shop->id);

        return $product;
    }
	
	public static function getProductPriceById($id_product){
        $product = new Product($id_product, true, Context::getContext()->language->id, Context::getContext()->shop->id);

        return $product->getPrice(true, NULL, 6);
    }
    ...
    */
	...
	public static function ReturnSizesInLine($data){
		$data = unserialize($data);

		foreach ($data['obj'] as $key => $val){
			$product  = new Product($key, true, Context::getContext()->language->id, Context::getContext()->shop->id);
			$string .= $product->name .' : Размер ' . $val.'</br>';
		}
    ...
    */
    public static function displayError($string = 'Fatal error', $htmlentities = true, Context $context = null)
    {
        ...
        if (defined('_PS_MODE_DEV_') && _PS_MODE_DEV_ && $string == 'Fatal error') {
            return ('<pre>'.print_r(debug_print_backtrace(), true).'</pre>');
        }
        if (!is_array($_ERRORS)) {
...
';
	...
	public static function getCartPricePack($id_product){
		$result = Db::getInstance()->executeS('SELECT id_product_item FROM `'._DB_PREFIX_.'pack` WHERE id_product_pack = '.(int)$id_product);
                
        $pack_items_custom = array();
		foreach ($result as $p){
			//var_dump($p['id_product_item']);
			
			$prod = new Product($p['id_product_item'], true, Context::getContext()->language->id, Context::getContext()->shop->id);
			$total += $prod->getPrice(true, NULL, 6);
			//var_dump($prod->getPrice(true, NULL, 6));
			//$total += $prod->price;
		}
		return $total;
	}
	
	public static function getCartPricePackCart($id_product){
		$result = Db::getInstance()->executeS('SELECT id_product_item FROM `'._DB_PREFIX_.'pack` WHERE id_product_pack = '.(int)$id_product);
		
			foreach ($result as $p){
				$sql = new DbQuery();
				$sql->select('product_shop.`price`, product_shop.`id_product`');
				$sql->from('product', 'p');
				$sql->innerJoin('product_shop', 'product_shop', '(product_shop.id_product=p.id_product AND product_shop.id_shop = '.(int)Context::getContext()->shop->id.')');
				$sql->where('p.`id_product` = '.(int)$p['id_product_item']);
