...
class CartCore extends ObjectModel
{
    ...
    public function getProducts($refresh = false, $id_product = false, $id_country = null)
    {
        ...
        // Build SELECT
        $sql->select('cp.`id_product_attribute`,cp.`sizes`, cp.`id_product`, cp.`quantity` AS cart_quantity, cp.id_shop, pl.`name`, p.`is_virtual`,
						pl.`description_short`, pl.`available_now`, pl.`available_later`, product_shop.`id_category_default`, p.`id_supplier`,
						p.`id_manufacturer`, product_shop.`on_sale`, product_shop.`ecotax`, product_shop.`additional_shipping_cost`,
						product_shop.`available_for_order`, product_shop.`price`, product_shop.`active`, product_shop.`unity`, product_shop.`unit_price_ratio`,
						stock.`quantity` AS quantity_available, p.`width`, p.`cache_is_pack`, p.`height`, p.`depth`, stock.`out_of_stock`, p.`weight`,
    ...
    public function updateQty($quantity, $id_product, $id_product_attribute = null, $id_customization = false,
        $operator = 'up', $id_address_delivery = 0, Shop $shop = null, $auto_add_cart_rule = true, $sizes = null)
    {
        if (!$shop) {
    ...
    */
    ...
    public function getAddressCollectionCustom()
    {
        $collection = array();
        $cache_id = 'Cart::getAddressCollection'.(int)$this->id;
        if (!Cache::isStored($cache_id)) {
    ...
    */
    ...
    public static function addExtraCarriersCustom(&$array)
    {
        $first = true;
        $hook_extracarrier_addr = array();
        foreach (Context::getContext()->cart->getAddressCollectionCustom() as $address) {
            $hook = Hook::exec('displayCarrierList', array('address' => $address));
            $hook_extracarrier_addr[$address->id] = $hook;

            return $hook_extracarrier_addr;
        }
    }
